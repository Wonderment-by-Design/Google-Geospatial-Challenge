...
public class ARCoreCustomCameraBackground : MonoBehaviour
{
    /// <summary>
    ...
    ARSemanticManager m_SemanticManager;
    Texture2D m_SemanticTexture;
    bool m_IsSemanticEnabled = false;
    ...
    protected ARSemanticManager semanticManager => m_SemanticManager;
    ...
    //public Material debugSemanticMaterial;
    ...
    void Awake()
    {
        ...
        m_SemanticManager = GetComponent<ARSemanticManager>();

        //debugSemanticMaterial.SetColorArray("_LabelColorArray", new LabelColorManager().GetColorArray());
    }

    private void Start()
    {
        Invoke("SemanticsTimeout", 5);
    }

    void SemanticsTimeout()
    {
        m_IsSemanticEnabled = true;
    }
    ...
    private void Update()
    {
        if(ARSession.state == ARSessionState.SessionTracking && m_IsSemanticEnabled)
            UpdateSemantics();
    }
    ...
    private void UpdateSemantics()
    {
        ...
        {
            if (semanticManager.TryGetSemanticTexture(ref m_SemanticTexture))
            {
                material.SetTexture("_Semantic", m_SemanticTexture);
            }
        }
    }
